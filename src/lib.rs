...
unsafe fn relocate(mmap: *mut u8, len: usize, relocation: &Relocation) {
  ...
  let x86callhwnd = |displacement: i64| {
  ...
  match relocation.kind {
    ...
    RelocKind::Abs8 => unimplemented!("Unsupported platform"),
    ...
    | RelocKind::X86PCRel4
    | RelocKind::X86CallPCRelOrPCRelProvidedRelativeBytes => unimplemented!("Unsupported platform"),
    #[cfg(target_arch = "x86_64")]
    RelocKind::X86CallPCRel4 | RelocKind::X86PCRel4 => {
      ...
      x86callhwnd(displacement);
    }
    #[cfg(target_arch = "x86_64")]
    RelocKind::X86CallPCRelOrPCRelProvidedRelativeBytes => {
      x86callhwnd(relocation.symbol_addr.cast_signed());
    }
    #[cfg(not(target_arch = "aarch64"))]
    RelocKind::Arm64Call | RelocKind::Arm64CallProvidedRelativeBytes => {
